...
var y = class {
	...
	advance(t) {
		...
		if (this.duration && this.easing) {
			...
			this.lerp
				? ((this.value = z(this.value, this.to, this.lerp * 60, t)), Math.round(this.value) === this.to && ((this.value = this.to), (e = !0)))
...
var L = class {
	...
	constructor({
		...
		(window.lenisVersion = k),
			...
			(this.virtualScroll = new T(i, { touchMultiplier: a, wheelMultiplier: v })),
	...
	onVirtualScroll = (t) => {
		...
		if ((this.emitter.emit("virtual-scroll", { deltaX: e, deltaY: i, event: s }), s.ctrlKey || s.lenisStopPropagation)) return;
		...
		let d = (this.options.gestureOrientation === "vertical" && i === 0) || (this.options.gestureOrientation === "horizontal" && e === 0);
		if (h || d) return;
	...
	onNativeScroll = () => {
		if (
			(this._resetVelocityTimeout !== null && (clearTimeout(this._resetVelocityTimeout), (this._resetVelocityTimeout = null)),
	...
	raf = (t) => {
		let e = t - (this.time || t);
		(this.time = t), this.animate.advance(e * 0.001), this.options.autoRaf && (this.__rafID = requestAnimationFrame(this.raf));
	};
	scrollTo(
		t,
		{
		...
		if (!((this.isStopped || this.isLocked) && !c)) {
			if (typeof t == "string" && ["top", "left", "start"].includes(t)) t = 0;
				...
				let n;
				if ((typeof t == "string" ? (n = document.querySelector(t)) : t instanceof HTMLElement && t?.nodeType && (n = t), n)) {
			...
			if (typeof t == "number") {
				if (
					((t += e),
					(t = Math.round(t)),
					this.options.infinite ? p && (this.targetScroll = this.animatedScroll = this.scroll) : (t = w(0, t, this.limit)),
					t === this.targetScroll)
				) {
					S?.(this), d?.(this);
					return;
				...
				p || (this.targetScroll = t),
					this.animate.fromTo(this.animatedScroll, t, {
						...
						lerp: h,
						onStart: () => {
							s && (this.isLocked = !0), (this.isScrolling = "smooth"), S?.(this);
						},
						onUpdate: (n, a) => {
							(this.isScrolling = "smooth"),
								...
								a || this.emit(),
								a && (this.reset(), this.emit(), d?.(this), (this.userData = {}), this.preventNextNativeScrollEvent());
						},
					});
			}
		}
